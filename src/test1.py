    ...
    def callback(self, rgb_msg, sem_msg, depth_msg):
        ...
        if main_roi is not None:
            ...
            if roi.size > 0:
                ...
                red_mask = ((h < 20) | (h > 150)) & (s > 50) & (v > 100)
                green_mask = (h > 35) & (h < 95) & (s > 50) & (v > 100)
                n_red = np.sum(red_mask)
                n_green = np.sum(green_mask)
                n_bright = np.sum(mask_bright)
                rospy.loginfo(f"[Main TL id={main_roi['id']}] ROI {main_roi['rect']}, area={main_roi['area']}, n_bright={n_bright}, n_red={n_red}, n_green={n_green}")

                # 集中法：谁亮色里像素多就判谁（且必须有亮度阈值和数量）
                if n_bright > 5 and n_red > n_green and n_red > 6:
                    detected_status = 0
                    rospy.loginfo(f"Red dominant: {n_red}/{n_bright}")
                elif n_bright > 5 and n_green > n_red and n_green > 6:
                    detected_status = 1
                    rospy.loginfo(f"Green dominant: {n_green}/{n_bright}")
                else:
                    rospy.loginfo("No dominant color found in ROI (too mixed or too dim).")
                # 输出深度信息
